...
HQ_MIN_RR     = st.sidebar.number_input("Full-alignment R:R mark",   value=1.5,  min_value=0.2,  step=0.1,
    help="R:R at which a name is tagged 'fully aligned' in the monitor. This is "
         "a state label for information only — the app does not treat it as a "
...
st.sidebar.divider()
st.sidebar.header("🛡️ Discipline Guardrails")
st.sidebar.caption("Soft warnings only — these never block a trade, they just "
                   "make sure you notice.")
MAX_TRADES_PER_DAY = int(st.sidebar.number_input("Max trades / day", value=3, min_value=1, max_value=50,
    help="Warn once you've logged this many trades today. Overtrading is a top "
         "cause of giving back gains."))
MAX_LOSSES_PER_DAY = int(st.sidebar.number_input("Max losses / day", value=2, min_value=1, max_value=20,
    help="Warn once you've taken this many losses today — the classic tilt zone."))
...
def journal_stats(journal: list) -> dict:
    if not journal:
        return {}
    # Only CLOSED trades count toward performance. OPEN trades (logged via the
    # Trade Setup tab but not yet resolved) are tracked separately so they don't
    # deflate win rate or crash the streak sort (they have no "closed" date).
    closed = [j for j in journal if j.get("outcome") in ("WIN", "LOSS", "BREAKEVEN")]
    n_open = len(journal) - len(closed)
    if not closed:
        return {"total": 0, "open": n_open}
    wins   = [j for j in closed if j["outcome"] == "WIN"]
    losses = [j for j in closed if j["outcome"] == "LOSS"]
    be     = [j for j in closed if j["outcome"] == "BREAKEVEN"]
    total  = len(closed)
    ...
    total_r  = round(sum(j["actual_rr"] for j in closed), 2)
    ...
    outcomes    = [j["outcome"] for j in sorted(closed, key=lambda x: x.get("closed", ""))]
    streak      = 0
...
# Telegram alerts were removed in the discipline-enforcer redesign. They were
# the strongest "act now" nudge in the app and fired on a signal that
# backtesting showed has no predictive edge, so they encouraged exactly the
# impulsive behaviour this tool now exists to curb.
...
# TOP-LEVEL TABS
# ─────────────────────────────────────────────
TAB_SCAN, TAB_SETUP, TAB_STOCK, TAB_ALERTS, TAB_JOURNAL = st.tabs([
    "📊 Watchlist Monitor", "🎯 Trade Setup", "🔍 Stock Analysis",
...
with TAB_SCAN:
    ...
    st.caption("Current technical state of your watchlist — neutral information, "
               "not a buy list. To act on any idea, use the 🎯 Trade Setup tab.")
    ...
    with sc1:
        st.caption(f"Tickers: {', '.join(SCAN_LIST)} · Cache: 5 min")
    with sc2:
        if st.button("🔄 Refresh state", type="primary", key="refresh_scan"):
    ...
    if not st.session_state["scan_triggered"]:
        st.info("👆 Click **Refresh state** to load the current technical state "
                "of your watchlist.")
    else:
        with st.spinner("Loading watchlist state…"):
        ...
        # NOTE (design): this is a NEUTRAL technical dashboard, not a buy list.
        # Backtesting showed this signal has no demonstrable predictive edge, so
        # the app no longer frames anything as a "high quality" trade to take.
        # It reports the current technical STATE of each watchlist name as
        # information — you decide what (if anything) to do with it.
        aligned  = [s for s in all_setups if s["all_pass"]]
        partial  = [s for s in all_setups if not s["all_pass"]]

        # Journal alignment states for later behavioural review (no alerts fired).
        for a in aligned:
        ...
        c1, c2 = st.columns(2)
        c1.metric("Fully aligned", len(aligned),
                  help="All base conditions + all 4 enhancement filters agree. "
                       "This is a state description, NOT a recommendation to buy.")
        c2.metric("Partial", len(partial))
        st.divider()

        st.caption(
            "⚠️ This is a **neutral technical dashboard**, not a buy list. "
            "Backtesting found no predictive edge in these signals — treat every "
            "row as *information about current price structure*, then apply your "
            "own judgment. Use the **🎯 Trade Setup** tab to pressure-test any "
            "idea before you act."
        )

        st.markdown("### Fully-aligned names")
        if aligned:
            for a in aligned:
                with st.container(border=True):
            ...
            st.info("No names currently have all conditions aligned.")

        with st.expander(f"Partial / mixed state ({len(partial)} names)"):
            for a in partial:
                failed = [n for n,f in a["filters"].items() if not f["pass"]]
                st.write(f"**{a['ticker']}** — {a['trend']} | RR {a['rr']} | "
                         f"Not aligned: {', '.join(failed) if failed else '—'}")
        ...
        if df is None:
            ...
            with stab1:
                ...
                if r.get("blocked"):
                    ...
                    badge = ("● Fully aligned" if r["high_quality"]
                             else "● All filters agree" if r["all_pass"]
                             else f"◐ Partial — {r['filters_pass']}/{r['filters_total']} filters")
                    st.markdown(f"### {badge} — {r['trend']} ({r['strength']})")
                    st.caption("State description, not a buy recommendation.")
                    s1,s2,s3,s4 = st.columns(4)
...
# ═══════════════════════════════════════════════
with TAB_SETUP:
    st.subheader("🎯 Trade Setup — pressure-test your own idea")
    st.caption(
        "You bring the trade; the app enforces the risk framework. It never "
        "blocks you — it surfaces every relevant warning so you never act "
        "*unaware* of a risk. The decision stays yours."
    )

    # ── Daily guardrail status (anti-tilt) ──
    today_iso = datetime.now(pytz.timezone("America/New_York")).strftime("%Y-%m-%d")
    jrnl_today = [j for j in load_journal()
                  if str(j.get("closed", "")).startswith(today_iso)]
    losses_today = sum(1 for j in jrnl_today if j.get("outcome") == "LOSS")
    trades_today = len(jrnl_today)

    g1, g2 = st.columns(2)
    with g1:
        if trades_today >= MAX_TRADES_PER_DAY:
            st.warning(f"⚠️ **{trades_today} trades logged today** "
                       f"(your limit: {MAX_TRADES_PER_DAY}). Overtrading is one of "
                       f"the most common ways to give back gains.")
        else:
            st.info(f"Trades logged today: {trades_today} / {MAX_TRADES_PER_DAY}")
    with g2:
        if losses_today >= MAX_LOSSES_PER_DAY:
            st.warning(f"🛑 **{losses_today} losses today** (your limit: "
                       f"{MAX_LOSSES_PER_DAY}). This is the classic tilt zone — "
                       f"consider stepping away rather than trading to get it back.")
        else:
            st.info(f"Losses today: {losses_today} / {MAX_LOSSES_PER_DAY}")
    ...
    with ic1:
        setup_ticker = st.text_input("Ticker", placeholder="NVDA",
                                     key="setup_ticker").strip().upper()
    with ic2:
        setup_dir = st.radio("Direction", ["Long", "Short"], key="setup_dir",
                             horizontal=True)
    with ic3:
        auto_levels = st.checkbox("Suggest levels from ATR", value=True,
                                  key="setup_auto")

    if setup_ticker:
        trend = "Bullish" if setup_dir == "Long" else "Bearish"

        # Pull current price + ATR for suggestions / context
        df_raw = get_data(setup_ticker)
        suggested = None
        if df_raw is not None:
            dfc = compute(df_raw)
            dfc, _ = drop_partial_bar(dfc)
            if has_sufficient_history(dfc, setup_ticker):
                price = float(dfc["Close"].iloc[-1])
                atr   = float(dfc["ATR"].iloc[-1])
                if trend == "Bullish":
                    s_entry = round(price, 2)
                    s_stop  = round(price - atr * ATR_STOP_MULT, 2)
                    s_tgt   = round(price + atr * ATR_TGT_MULT, 2)
                else:
                    s_entry = round(price, 2)
                    s_stop  = round(price + atr * ATR_STOP_MULT, 2)
                    s_tgt   = round(price - atr * ATR_TGT_MULT, 2)
                suggested = (s_entry, s_stop, s_tgt, price, atr)

        # Level inputs (prefilled from suggestion when available)
        d_entry, d_stop, d_tgt = (suggested[0], suggested[1], suggested[2]) \
            if (auto_levels and suggested) else (0.0, 0.0, 0.0)

        lc1, lc2, lc3 = st.columns(3)
        entry = lc1.number_input("Your entry",  value=float(d_entry), step=0.01,
                                 format="%.2f", key="setup_entry")
        stop  = lc2.number_input("Your stop",   value=float(d_stop),  step=0.01,
                                 format="%.2f", key="setup_stop")
        target= lc3.number_input("Your target", value=float(d_tgt),   step=0.01,
                                 format="%.2f", key="setup_target")

        if suggested:
            st.caption(f"Current price \\${suggested[3]:,.2f} · ATR \\${suggested[4]:,.2f} "
                       f"· ATR-suggested levels shown above (editable).")
        ...
        if entry > 0 and stop > 0 and target > 0:
            ...
            if trend == "Bullish" and not (stop < entry < target):
                warnings_list.append(
                    "Levels don't form a valid **long** (need stop < entry < target).")
            if trend == "Bearish" and not (target < entry < stop):
                warnings_list.append(
                    "Levels don't form a valid **short** (need target < entry < stop).")
            ...
            if rr < MIN_RR:
                warnings_list.append(
                    f"Reward:Risk is **{rr}**, below your minimum of {MIN_RR}. "
                    f"The target is close relative to the stop.")

            # Position sizing
            ps = calc_position_size(entry, stop)

            # Earnings blackout
            e_ok, e_detail = check_earnings_blackout(setup_ticker)
            if not e_ok:
                warnings_list.append(f"**Earnings risk** — {e_detail}.")

            # Daily guardrails
            if trades_today >= MAX_TRADES_PER_DAY:
                warnings_list.append(
                    f"You've already logged {trades_today} trades today "
                    f"(limit {MAX_TRADES_PER_DAY}).")
            if losses_today >= MAX_LOSSES_PER_DAY:
                warnings_list.append(
                    f"You've taken {losses_today} losses today "
                    f"(limit {MAX_LOSSES_PER_DAY}) — tilt risk.")

            # ── Trade plan summary ──
            m1, m2, m3, m4 = st.columns(4)
            m1.metric("Entry",  f"${entry:,.2f}")
            m2.metric("Stop",   f"${stop:,.2f}")
            m3.metric("Target", f"${target:,.2f}")
            m4.metric("R:R",    rr)

            if ps["affordable"]:
                st.info(
                    f"💰 **Size for your rule** — risk \\${ps['risk_dollars']:,.2f} "
                    f"({RISK_PCT}% of \\${ACCOUNT_SIZE:,}) → **{ps['shares']} shares** "
                    f"or **{ps['contracts']} option contract(s)**.")
            else:
                st.warning(
                    f"⚠️ Options exceed your risk limit at this stop.\n\n{ps['note']}")

            st.caption(
                "⚠️ Gap risk: a stop is not guaranteed. An overnight gap can fill "
                "beyond it, so realised loss can exceed the planned risk above.")

            # ── Soft-warning panel ──
            if warnings_list:
                st.markdown("#### ⚠️ Before you take this trade")
                for w in warnings_list:
                    st.markdown(f"- {w}")
                st.caption("These are warnings, not blocks. You can still log the "
                           "trade — just do it knowing what's flagged.")
            else:
                st.success("✅ No risk flags. Levels are coherent, R:R clears your "
                           "minimum, no earnings or daily-limit issues.")

            # ── Pre-trade checklist (informational, never blocks) ──
            with st.expander("📋 Pre-trade checklist"):
                st.checkbox("I have a clear thesis for this trade", key="chk_thesis")
                st.checkbox("My stop is where the thesis is proven wrong", key="chk_stop")
                st.checkbox("Size is within my risk rule", key="chk_size")
                st.checkbox("I've checked earnings / event risk", key="chk_earn")
                st.checkbox("I'm not trading to recover a loss", key="chk_tilt")

            # ── Log it ──
            thesis = st.text_input("Trade thesis (optional, for your journal)",
                                   key="setup_thesis")
            if st.button("📓 Log this trade to journal", type="primary",
                         key="log_setup"):
                journal = load_journal()
                journal.append({
                    "id":       f"{setup_ticker}_{int(time.time())}",
                    "ticker":   setup_ticker,
                    "trend":    trend,
                    ...
                    "outcome":  "OPEN",
                    "actual_rr": 0.0,
                    "warnings_at_entry": warnings_list,
                    "source":   "manual_setup",
                })
                save_journal(journal)
                st.success(f"Logged {setup_ticker} {setup_dir}. Track the outcome "
                           f"in the 📓 Trade Journal tab.")
        else:
            st.info("Enter your entry, stop, and target to pressure-test the trade.")
    else:
        st.info("Enter a ticker to begin. The app will suggest ATR-based levels you "
                "can accept or override with your own.")
